...
### 4. Multi-Query Positional Arguments & Multi-Hit Boosting
...
- **How it works**: Providing multiple positional query arguments (`notebrain search "arg1" "arg2"`) instructs NoteBrain to embed all query terms simultaneously using `emb.EmbedBatch`. Candidates are retrieved independently for each query vector across ChromaDB using semantic vector similarity (cosine distance).
